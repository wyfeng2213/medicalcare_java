...
<title>用户收藏管理</title>
    ...
    $(function() {
        ...
        dataGrid = $('#dataGrid').datagrid({
            url : '${path }/crm/favorMedia/findFavorPage.do',
            fit : true,
            striped : true,
            rownumbers : true,
            pagination : true,
    ...
    function deleteFun(id) {
        ...
        parent.$.messager.confirm('询问', '您是否要删除当用户收藏？', function(b) {
            if (b) {
                var currentUserId = '${sessionInfo.id}';/*当前登录用户的ID*/
                if (currentUserId != id) {
                    progressLoad();
                    $.post('${path }/crm/favorMedia/delete.do', {
                        id : id
                    }, function(result) {
                        if (result.success) {
...
<div data-options="region:'north',border:false" style="height: 30px; overflow: hidden;background-color: #fff">
    ...
    <div data-options="region:'center',border:true,title:'用户收藏列表'" >
