...
import com.cmcc.medicalcare.utils.Base64Utils;
...
public class SecretaryLoginInfoController {
	...
	public Results<Map<String, Object>> login(HttpServletRequest request, String hUserName, String hUserPassword,
			@RequestParam String verifyCode) {
		Results<Map<String, Object>> results = new Results<Map<String, Object>>();
		Map<String, Object> map = new HashMap<String, Object>();
		// Integer count = (Integer)
		// request.getSession().getAttribute(hUserName);
		// if (count == null) {
		// count = 0;
		// }
		// if (count >= 5) {
		...
		if (!imgVcode.equalsIgnoreCase(verifyCode)) {
			results.setCode(MessageCode.CODE_404);
			results.setMessage("图片验证码错误");
			return results;
		}
		if (secretaryUser == null) {
			results.setCode(MessageCode.CODE_404);
			results.setMessage("用户或密码错误");
			return results;
		}
		
		String password = secretaryUser.getPassword();
		password = MD5.encode(password, "UTF-8");
//		System.out.println("password::"+password);
//		System.out.println("hUserPassword::"+hUserPassword);
		if (null == password || !password.equalsIgnoreCase(hUserPassword)) {
			results.setCode(MessageCode.CODE_404);
			results.setMessage("用户或密码错误");
			return results;
		}
		try {
			// if (secretaryUser != null &&
			// secretaryUser.getPassword().equals(hUserPassword)
			// && imgVcode.equalsIgnoreCase(verifyCode)) {
			request.getSession().setAttribute("sessionId", request.getSession().getId());

			// 登录成功后,记录登录信息
			SecretaryLoginInfo secretaryLoginInfo_ = secretaryLoginInfoService.findByParam("selectBySecretaryId",
					secretaryUser.getId());
			if (null == secretaryLoginInfo_) {
				secretaryLoginInfo_ = new SecretaryLoginInfo();
				secretaryLoginInfo_.setSecretaryId(secretaryUser.getId());
				secretaryLoginInfo_.setSecretaryName(secretaryUser.getName());
				secretaryLoginInfo_.setSecretaryPhone(secretaryUser.getPhone());
				secretaryLoginInfo_.setSecretaryLoginId(secretaryUser.getLoginId());
				secretaryLoginInfo_.setToken(request.getSession().getId());
				secretaryLoginInfo_.setLoginTime(new Date());
				secretaryLoginInfo_.setIp(HTTPUtils.getIpAddrByRequest(request));
				secretaryLoginInfo_.setCreatetime(new Date());

				secretaryLoginInfoService.insert(secretaryLoginInfo_);
			} else {
			...
			// request.getSession().setAttribute("secretaryUser",
			// secretaryUser);
			String password_ =Base64Utils.encode(secretaryUser.getPassword().trim().getBytes());
			secretaryUser.setPassword(password_);
			map.put("secretaryUser", secretaryUser);
	...
	public Results<String> sendCode(HttpServletRequest request, @RequestParam String phone) {
		...
		if (Toolkit.isMobileNO(phone)) {
			...
			boolean flag = ShuyuanUtils.sendValcode(phone);
			if (true == flag) {
				results.setCode(MessageCode.CODE_200);
			...
			results.setMessage("只能输入正确的手机号码！");
		}
	...
	public Results<Map<String, Object>> codeLogin(HttpServletRequest request, @RequestParam String phone,
		...
		if (Toolkit.isMobileNO(phone)) {
			// 向服务器发送验证码验证请求
			Map<String, Object> flagMap = ShuyuanUtils.checkValcode(phone, code);
