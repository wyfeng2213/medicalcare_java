...
public class FileFilterUtils {
  ...
  //根据文件路径判断文件类型是否MediaFile
    public static boolean isMediaFileType(String path) {  //自己增加
        MediaFileType type = getFileType(path);
        if(null != type) {
            return isMediaFileType(type.fileType);
        }
        return false;
    }
     ...
     * @return
     */
    public static boolean getFileTypeForMimeType(String path,String suffix) {
		int lastDot = path.lastIndexOf(".");
		if (lastDot < 0)
			return false;
		String suffix_ = path.substring(lastDot + 1).toUpperCase();
		suffix = suffix.toUpperCase();
		return (suffix_.equals(suffix));
    }
}